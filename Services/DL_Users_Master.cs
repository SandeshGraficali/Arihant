...
namespace Arihant.Services
{
    public class DL_Users_Master
    {
        ...
        public VendorMasterVM GetVendorByID(int id)
        {
            ...
            var data= ds.Tables.Map<VendorMasterVM>().ToList();
        ...
        public List<VendorMasterVM> GetVendorList()
        {
            List<VendorMasterVM> vendors = new List<VendorMasterVM>();
            try
            {
                var parameters = new Dictionary<string, SqlParameter>
                {
                ...
                if (response != null && response.Tables.Count > 0)
                {
                    ...
                    {
                        vendors.Add(new VendorMasterVM
                        {
        ...
        public ClientSubmissionViewModel GetClientById(int clientId)
        {
            ...
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables.Count > 1 && ds.Tables[1].Rows.Count > 0)
                {
                    ...
                    model.Email = con["Email"].ToString();
                    model.AlternateEmail = con["AlternetEmail"].ToString();
                    model.ContactPersonName = con["ContactPersonName"].ToString();
                    model.OffMobileNO = con["OfficeMobileNO"].ToString();
                    model.WhatsappNo = con["WhatsAppNO"].ToString();
                   model.Website = con["Website"].ToString();
                    model.Tel1 = con["Telephone"].ToString();
                    model.MobileNo = con["MobileNumber"].ToString();
                ...
                {
                    foreach (DataRow adr in ds.Tables[2].Rows)
                    {
                        model.AddressList.Add(new AddressDTO
                        {
                            AddressTypeID = adr["AddressType"].ToString(),
        ...
        public string AddClientDetails(string data, string CreatedBy)
        {
            try
            {
                ...
                var parameters = new Dictionary<string, SqlParameter>
                {
                    ...
                    { "Telephone", new SqlParameter("@Telephone", model?.Tel1 ?? (object)DBNull.Value) },
                    { "Mobile1", new SqlParameter("@Mobile1", model.MobileNo.ToString())  },
                      { "AddressJson", new SqlParameter("@AddressJson", addressJson) },
                    { "CreatedBy", new SqlParameter("@CreatedBy", CreatedBy) },
                    { "result", outParam }
                };
        ...
        public List<CompanyProfileModel> getCompanyByID(string id)
        {
            CompanyProfileModel company = null;
            var parameters = new Dictionary<string, SqlParameter>
            {
                { "Operation", new SqlParameter("@Operation", "Get_Company_MasterByID") },
                { "ID", new SqlParameter("@ID", id) }
            };
            ...
            DataSet ds = gc.ExecuteStoredProcedureGetDataSet("SP_Update_Company", parameters.Values.ToArray());
             ...
             var responce= ds.Tables.Map<CompanyProfileModel>().ToList();
        ...
        public List<CompanyProfileModel> GetAllCompanyMasters()
        {
           ...
           var newdata= ds.Tables.Map<CompanyProfileModel>().ToList();
        ...
        public string SaveCompanyDetails(string data , string CreatedBy)
        {
            try
            {
                ...
                var parameters = new Dictionary<string, SqlParameter>
                {
                    ...
                    { "AddressJson", new SqlParameter("@AddressJson", addressJson) },
                    { "BankJson", new SqlParameter("@BankJson", bankJson) },
                    { "Mobile1", new SqlParameter("@Mobile1", model.MobileNo )},
        ...
        public string UpdateCompanyDetails(string data , string Modifiedby)
        {
            try
            {
                ...
                var parameters = new Dictionary<string, SqlParameter>
                {
                    ...
                    { "ModifiedBy", new SqlParameter("@ModifiedBy", Modifiedby.ToString() ) },
                      { "AddressJson", new SqlParameter("@AddressJson", addressJson) },
                    { "BankJson", new SqlParameter("@BankJson", bankJson) },
